...
#define MODEL_FILE_TRASH    "/userdata/models/cnn_trash_classify/trash_detect.wk"
...
static HI_S32 CnnTrashClassifyFlag(const RecogNumInfo items[], HI_S32 itemNum, HI_CHAR* buf, HI_S32 size)
{
    ...
    for (HI_U32 i = 0; i < itemNum; i++) {
        ...
        switch (item->num) {
            case 0u:
                trashName = "cardboard";
                UartSendRead(uartFd1, cardboard); // cardboard
                SAMPLE_PRT("----trash name----:%s\n", trashName);
                break;
            case 1u:
                trashName = "glass";
                UartSendRead(uartFd1, glass); // glass
                SAMPLE_PRT("----trash name----:%s\n", trashName);
                break;
            case 2u:
                trashName = "paper";
                UartSendRead(uartFd1, paper); // paper
                SAMPLE_PRT("----trash name----:%s\n", trashName);
                break;
            case 3u:
                trashName = "plastic";
                UartSendRead(uartFd1, plastic); // plastic
                SAMPLE_PRT("----trash name----:%s\n", trashName);
                break;
            /* case 4u:
            case 5u:
