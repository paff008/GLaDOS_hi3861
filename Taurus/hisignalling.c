...
void UartSendRead(int fd, refuseClassification refuseType)
{
    ...
    // unsigned char uart_type[4] = {0, 2, 0, 3};
#ifdef  EXPANSION_BOARD
     switch (1) {
        /* case FistGesture:
        ...
        case cardboard:
            ...
            break;
         case glass:
            HisignallingMsgSend(fd, writeBuffer2, sizeof(writeBuffer2)/sizeof(writeBuffer3[0]));
            break;
         case paper:
            HisignallingMsgSend(fd, writeBuffer2, sizeof(writeBuffer2)/sizeof(writeBuffer4[0]));
            break;
         case plastic:
            HisignallingMsgSend(fd, writeBuffer2, sizeof(writeBuffer2)/sizeof(writeBuffer5[0]));
            break;
    } 
