...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionControlStatus,
                            payload->reportAction.controlActionStatus);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionMoveStatus,
                            payload->reportAction.moveActionStatus);   
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionMoveType,
                            payload->reportAction.moveTypeData);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionRubbishType,
                            payload->reportAction.rubbishTypeData);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionDistance,
                            payload->reportAction.distanceData);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionError,
                            payload->reportAction.errorData);                                           
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
...
#define CN_PROFILE_TOPICFMT_TOPIC            "$shadow/operation/JIFGEUU5BK/hi3861_GLaDOS"
int IoTProfilePropertyReport(char *deviceID, WeChatProfile *payload)
{
    ...
    if ((deviceID == NULL) || (payload== NULL)) {
        printf("IoTProfilePropertyReport 失败 1 \n");
        return ret;
    }
    topic = MakeTopic(CN_PROFILE_TOPICFMT_TOPIC, deviceID, NULL);
    if (topic == NULL) {
        printf("IoTProfilePropertyReport 失败 2 \n");
        return;
    }
    ...
    printf("IoTProfilePropertyReport 成功 \n");
    hi_free(0, topic);
    cJSON_free(msg);
    printf("cjson he topic 释放完毕\n");
