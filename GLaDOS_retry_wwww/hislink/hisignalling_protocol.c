...
osMessageQueueId_t MSG_Taurus;
Msg_Taurus msg_taurus;
...
HisignallingErrorType HisignallingMsgReceive(hi_u8 *buf, hi_u32 len)
{
    ...
    // unsigned char textbuff[7] = {0xaa, 0x55, 0x0, 0x2, 0x0, 0x8, 0xff};
    ...
    /* 校验crc */
     if (len > HISGNALLING_MSG_CRC32_LEN) {
        ...
        if (((hi_u8)((crcCheckReceived & 0xff000000)>>RIGHT_MOVE_24_BIT) != buf[len - 4]) && // 4: addr offset 
            ...
            printf("\r\n");
            //return HISIGNALLING_RET_VAL_ERROR;                          //在正式时要打开的！！！！！！！！！！！！！！！！！
        }
    }
    HISIGNALLING_LOG_ERROR("<Hi3861>: crc32 Verification succed!\n");
    for (int i = 0; i < len; i++) {
        printf("0x%x ", buf[i]);
    }
    printf("\r\n");
    hi_u8 check_1 = 0x00;
    if(buf[2] == check_1){                    //此处处理收到的hi3516的数据，下面例子为通过信号来将数据发送到msg_find中进行下一步处理
        msg_taurus.buf_x = buf[3];
        msg_taurus.buf_y = buf[4];
        msg_taurus.buf_type = buf[5];
        osMessageQueuePut(MSG_Taurus, &msg_taurus, 0U, 0U);
        osThreadYield();                                            // suspend thread   
    }else{
        printf("receive from taurus error1");
    }
...
hi_void *HisignallingMsgHandle(void)  //char *param
{
